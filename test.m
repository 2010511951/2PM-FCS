...
% Tmin1 = 40e-6*bin_size;
% %T1 = [10e-6 12e-6 15e-6 20e-6 30e-6 40e-6 T(1:9) 0.01 0.02 0.03 0.1 1]; 
% T1 = Tmin1.*2.^(1:0.1:10);
% Ksq_2P = Ksq_2P_ofAPDSignal(data, T1, Tmin1, ds);
% figure('color', 'w')
% plot(log10(T1), Ksq_2P);
% 
% %tau_n = round(T1./ds);
% tau_n = [10.^(0:0.1:6)]; 
% %tau_n = 1:1e4;
% tau = tau_n.*ds;
% g2 = g2autocorr_c(data, 0, tau_n);
% figure('color', 'w')
% plot(log10(tau), g2);
% 
% figure('color', 'w')
% plot(log10(tau), g2/2 + mean(g2(1:1))/2 - 1); hold on
% plot(log10(T1), Ksq_2P); hold off
% 
% figure('color', 'w')
% plot(log10(tau), maxmin_normalize(g2)); hold on
% plot(log10(T1), maxmin_normalize(Ksq_2P, [Ksq_2P(end) Ksq_2P(1)])); hold off
